...
entropy = sum(-p * log2(p) for (_, p) in freq)

# Calcular longitud media del código de Huffman
avg_huffman_length = sum(len(huffmanCode[char]) * p for (char, p) in freq)

# Calcular varianza del código de Huffman
variance = sum((len(huffmanCode[char]) - avg_huffman_length) ** 2 * p for (char, p) in freq)

# Calcular eficiencia de la codificación original de la fuente
source_encoding_efficiency = entropy / 8  # Suponiendo que los caracteres se codifican con 8 bits cada uno

# Calcular eficiencia del nuevo código generado
new_encoding_efficiency = avg_huffman_length / 8  # Suponiendo que los caracteres de Huffman se codifican con 8 bits cada uno

# Mostrar los resultados en consola
print("Entropía de la fuente:", entropy, "bits")
print("Longitud media del código de Huffman:", avg_huffman_length, "bits")
print("Varianza del código de Huffman:", variance)
print("Eficiencia de la codificación original de la fuente:", source_encoding_efficiency)
print("Eficiencia del nuevo código generado:", new_encoding_efficiency)
...
def generar_bin(lista, nombre_archivo):
    try:
        with open(nombre_archivo, 'wb') as archivo:
            pickle.dump(lista, archivo)
        print("Lista escrita en el archivo binario correctamente.")
